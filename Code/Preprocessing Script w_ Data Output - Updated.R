...
Yeo_Johnson <- function(Data_in){
  affected_cols <- Data_in %>%
  ...
  # Bake the data
  transformed_data <- bake(transformed_recipe, new_data = Data_in)
  ...
  par(mfrow=c(1, 2))
  hist(Data_in$InternetUsersPct, main = "Original Internet Users", 
  ...
  sum(Data_in$InternetUsersPct < 0, na.rm = TRUE)
...
### FIRST - DO THE PREPROCESSING THAYT CAN BE DONE ON THE ENTIRE SET 
...
train_countries <- PreProcessingData %>%
  ...
  reframe(CountryCode = {
    ...
    sample_frac(cur_data(), 0.7)$CountryCode
  }) %>%
  pull(CountryCode)
...
# Fix the skewness
train_data <- Yeo_Johnson(train_data)
...
train_data <-impute_time_trend(train_data, HealthSpend, 0, Inf)
train_data <-impute_time_trend(train_data, GovtEduSpendPctGDP, 0, 100)
train_data <-impute_time_trend(train_data, GovernanceScore, -2.5, 2.5)
...
# Save train_data and test_data as .rds to be loaded in other files
# Save them as .xlsx files to be loaded into python for deep-learning
...
saveRDS(PCAData, file = "PCAData.rds")
